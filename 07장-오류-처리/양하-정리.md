...
class JiraErrorCode(Enum):
    ...
    def message(self) -> str:
        messages = {
            self.BAD_REQUEST: "잘못된 요청입니다 - 요청 형식이 올바르지 않거나 필수 파라미터가 누락되었습니다",
            self.UNAUTHORIZED: "인증에 실패했습니다 - 유효한 인증 정보가 필요합니다",
            self.FORBIDDEN: "접근 권한이 없습니다 - 이 리소스에 대한 접근 권한이 없습니다",
            self.NOT_FOUND: "리소스를 찾을 수 없습니다 - 요청한 리소스가 존재하지 않습니다",
            self.METHOD_NOT_ALLOWED: "허용되지 않는 메서드입니다 - 이 엔드포인트에서 사용할 수 없는 HTTP 메서드입니다",
            self.CONFLICT: "충돌이 발생했습니다 - 리소스의 현재 상태와 요청이 충돌합니다",
            self.GONE: "리소스가 더 이상 사용할 수 없습니다 - 요청한 리소스가 영구적으로 제거되었습니다",
            self.UNSUPPORTED_MEDIA_TYPE: "지원되지 않는 미디어 타입입니다 - 요청의 Content-Type이 지원되지 않습니다",
            self.UNPROCESSABLE_ENTITY: "처리할 수 없는 엔티티입니다 - 요청 데이터의 유효성 검사에 실패했습니다",
            self.TOO_MANY_REQUESTS: "요청이 너무 많습니다 - API 호출 한도를 초과했습니다",
            self.INTERNAL_SERVER_ERROR: "내부 서버 오류가 발생했습니다 - JIRA 서버에서 예상치 못한 오류가 발생했습니다",
            self.NOT_IMPLEMENTED: "구현되지 않았습니다 - 요청한 기능이 서버에서 지원되지 않습니다",
            self.BAD_GATEWAY: "잘못된 게이트웨이입니다 - JIRA 서버가 다른 서버로부터 잘못된 응답을 받았습니다",
            self.SERVICE_UNAVAILABLE: "서비스를 사용할 수 없습니다 - JIRA 서버가 일시적으로 사용할 수 없는 상태입니다",
            self.GATEWAY_TIMEOUT: "게이트웨이 시간 초과입니다 - JIRA 서버가 다른 서버로부터 응답을 받지 못했습니다"
        }
...
class JiraApiClient:
    ...
    def _handle_response(self, response: requests.Response) -> Dict:
        ...
        error_msg = f"{error_code.message}: {error_message}"
        raise JiraApiError(response.status_code, error_msg)
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.ConnectionError):
                raise JiraApiError(JiraErrorCode.SERVICE_UNAVAILABLE.value, JiraErrorCode.SERVICE_UNAVAILABLE.message)
            elif isinstance(e, requests.exceptions.Timeout):
                raise JiraApiError(JiraErrorCode.GATEWAY_TIMEOUT.value, JiraErrorCode.GATEWAY_TIMEOUT.message)
            else:
                raise JiraApiError(JiraErrorCode.INTERNAL_SERVER_ERROR.value, JiraErrorCode.INTERNAL_SERVER_ERROR.message)
        except JiraApiError:
            raise
